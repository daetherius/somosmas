...
function _typmap($value, $in = 'type', $out = 'model'){
	...
	switch($out){
		case 'model':
			if($in == 'type'){
				foreach($sections as $idx => $sect){
					if(Inflector::slug($sect) == $value)
						return ucfirst($idx);
				}
			} else {
				return array_search($value,$sections);
			}
		break;
		...
		case 'label':
			...
			}elseif(isset($sections[strtolower($value)])){
				return ucfirst($sections[strtolower($value)]);
			}
		break;

		case 'type':
			if($in == 'model'){
				return Inflector::slug($sections[strtolower($value)]);
			} else {
