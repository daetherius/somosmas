...
////
	...
	formtips = new Tips('.formtipCaller',{
		...
		onShow: function(el,caller){ // En Moo 1.2.4 se requiere hackear/agregar el segundo param dentro del more.
			...
			el.set('tween',{ duration:250, link:'cancel', property:'opacity' }).tween(.8);
		},
		onHide: function(el){
	...
	myTips = new Tips('.tipCaller',{
		...
		onShow: function(el){
			var title = el.getElement('.tip-title');
			if(title.get('html')=='') title.setStyle('display','none');
			el.set('morph',{ duration:450, link:'cancel', transition:'back:out' }).morph({ 'margin-top':[-15,0], 'opacity':[0,1] });
		},
		onHide: function(el){ if(el.getStyle('opacity') > 0) el.morph({ 'margin-top':[0,-15], 'opacity':[1,0] }); }

	});

function mooPop(msg){
	var pop = new Element('div',{ 'class':'mooPop'}).inject(document.body,'top');
	pop.makeDraggable();
	new Element('a',{ href:'javascript:;','class':'mooPopClose',html:'<img src="/img/admin/popClose.gif" alt="Cerrar" />', events:{ click:function(){ this.getParent('.mooPop').nix(); }}}).inject(pop);
