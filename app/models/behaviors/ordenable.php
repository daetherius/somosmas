...
class OrdenableBehavior extends ModelBehavior{
	...
	function setup(&$model, $settings = array()){
		$this->settings[$model->alias] = am(array('group'=>true),$settings);
		
		if(sizeof($model->belongsTo)){
			if($this->settings[$model->alias]['group']===true){
				$parent = reset($model->belongsTo);
				$this->settings[$model->alias]['group'] = $parent['foreignKey'];
			}
		} else {
			$this->settings[$model->alias]['group'] = false;
		}
	}
