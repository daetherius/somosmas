...
class AppModel extends Model {
	...
	function __construct($id = false, $table = null, $ds = null){
		...
		foreach($this->_schema as $fieldName => $fieldData){
			...
			$this->_schema[$fieldName]['label'] = ucfirst($fieldLabel ? $fieldLabel : $fieldName);
		}
		
		/// Auto categoryModel
		if($this->belongsTo){
			foreach($this->belongsTo as $belongModel => $belongData){
				if(strpos(low($belongModel),'category')!== false){
					$this->categoryModel = $belongModel;
					break;
				}
			}
		}
		...
		if((!$this->skipValidation) && (strpos(strtolower($this->alias),'carousel')!==false || strpos(strtolower($this->alias),'img')!==false)){
		...
		foreach($rules as $field => $rule){
			...
			if($this->hasField($field) && (!in_array($field, $this->skipValidation))){
		...
		if($this->categoryModel){
			$belongs = $this->belongsTo[$this->categoryModel];
	...
	function afterSave($created){
		...
		Cache::write('last_update',date('d-m-Y H:i:s'),'updates');
	}
	...
	function hasCategory($check){
		if(!$this->categoryModel) return false;
		
		$field = key($check);
		$value = $check[$field];
		return $value || (isset($this->data[$this->categoryModel]['nombre']) && $this->data[$this->categoryModel]['nombre']);
	}
	...
	function deleteShortCache(){
		...
		foreach($paths as $path){
			...
			$files = $cacheFolder->find('^(.)*\_'.strtolower($this->name).'\_(.)*');
