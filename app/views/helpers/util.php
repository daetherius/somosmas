...
class UtilHelper extends AppHelper {
	...
	function th($item, $model = false, $opts = array()){
		$th = $footer = '';
		$resizeopts = array();
		$opts = array_merge(array('id'=>'','class'=>'','atts'=>array(),'url'=>false,'field'=>'src','w'=>0,'h'=>0,'descripcion'=>'nombre','fill'=>false,'footer'=>false),$opts);
		...
		//if(array_key_exists($opts['field'],$item)){
			...
			if($opts['descripcion']){
				if(!is_string($opts['descripcion']))
					$opts['descripcion'] = 'descripcion';
				
				if(isset($item[$opts['descripcion']]) && $item[$opts['descripcion']])
					$opts['atts']['title'] = $item[$opts['descripcion']];
			}
			...
			if($opts['url']){
				$th = $this->Html->link($th.$footer,$opts['url'],$opts['atts']);
			} else
				$th = $this->Html->div(null,$th.$footer,$opts['atts']);
	...
	function ofuscar($text, $emailto = true, $atts) {
		if(is_array($text)){
				...
				if(isset($it[$opts['model']]['externo']) && $it[$opts['model']]['externo']){
					...
					if($filtermode){
						...
						$url = am(array(
							...
							'action'=>$opts['action'],
							isset($it[$opts['model']]['slug']) ? $it[$opts['model']]['slug']:$it[$opts['model']]['id']
						), $opts['urlmerge']);
					}
...
*/
    function youtube($url,$mode = 'player',$HTMLatts=array()){
		...
		$yid = $ytArgs['v'];
		$HTMLatts = array_merge(array('class'=>'vPlayer','width'=>560,'height'=>315),$HTMLatts);
...
*/
	...
	function swf($url, $HTMLatts = array()){
		$w = $h = 0;
		$url = (strpos($url,'http://')=== 0 ? '':'/').$url;

		if(!isset($HTMLatts['width']) && !isset($HTMLatts['height'])){
			list($HTMLatts['width'],$HTMLatts['height']) = @getimagesize((strpos($url,'http://')=== 0 ? '':'.').$url);
		} elseif(isset($HTMLatts['width']) && $HTMLatts['width'] && (!isset($HTMLatts['height']) || !$HTMLatts['height'])){
			list($HTMLatts['width'],$HTMLatts['height']) = $this->resize($url,$HTMLatts['width']);
		}

		return '<object '.($this->serializeHTML($HTMLatts)).' data="'.$url.'" type="application/x-shockwave-flash"><param name="movie" value="'.$url.'"></param><param name="wMode" value="transparent"></param></object>';
	}
