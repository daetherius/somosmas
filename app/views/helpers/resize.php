...
*/
	function resize($url, $opts = array()) {
		...
		if ($opts['aspect']) { // adjust to aspect
			// 2 / 3 [0.66] > 3 / 5 [0.6]
			if (($size[1]/$opts['h']) > ($size[0]/$opts['w'])){ // $size[0]:width, [1]:height, [2]:type
				if($opts['fill']) // fill space determined by dimensions
		...
		$resize = (!$cached) ? (($size[0] > $opts['w'] || $size[1] > $opts['h']) || ($size[0] < $opts['w'] || $size[1] < $opts['h'])) : false;
