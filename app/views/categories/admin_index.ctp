...
echo $this->element('adminhdr',array('links'=>array(array('text'=>'Agregar','href'=>false,'class'=>'add','atts'=>array('id'=>'elistAdder')))));
...
echo
	$html->div('OrderContainer'),
		...
		$moo->elist($_m[0],
			...
			array(
				'zoom'=>$parent ? false : $m->asTree,
				'data'=>$orderdata,
				'sort'=>1,
				'adder'=>'elistAdder',
				'remover'=>1,
				'min'=>0,
				'custom'=>array(array('text'=>'Editar','action'=>'admin_editar')),
				'confirmdelete'=>1
				),
			array('id'=>$_m[0].'_elist')
		),
		$parent ? $form->input('parent_id',array('value'=>$parent,'type'=>'hidden')):'',
		$form->end(),
	'</div>';
?>