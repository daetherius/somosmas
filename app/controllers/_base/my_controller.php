...
class MyController extends AppController{
	function __construct(){
		parent::__construct();
		if(is_array($this->uses) && $this->uses){
			foreach($this->uses as $model){
				$this->loadModel($model);
				$order = $model.'.id DESC';
					
				if($this->$model->hasField('orden')){
					$order = $model.'.orden DESC, '.$order;
				} elseif($this->$model->hasField('created')){
					$order = $model.'.created DESC, '.$order;
				}
				...
				if(isset($this->paginate[$model]))
					$paginate = array_merge($paginate, $this->paginate[$model]);
					
				$this->paginate[$model] = $paginate;
			}
		}
	}
	...
	function admin_toggle($id = false, $field = 'activo', $exclusivo = false){
		...
		if($this->m[0]->hasField($field) && $current = $this->m[0]->read($field,$id)){
			...
			$this->m[0]->create();
			$this->m[0]->id = $id;
			$this->m[0]->saveField($field,!$current[$this->uses[0]][$field]);
		}
	...
	function last(){
		...
		if(isset($this->params['isAjax']) && $this->params['isAjax'] && ((int)$params['json']!==0))
